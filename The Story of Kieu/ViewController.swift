...
class ViewController: UIViewController, UITextFieldDelegate {
    ...
    @IBAction func typing(_ sender: UITextField) {
        guard let text = sender.text, let index = Int(text) else {
            return
        }
        highlightIndex = []
        ...
        if index >= content.count {
            calculateIndex = content.count
        }
        if calculateIndex % 2 == 0 {
            calculateIndex -= 1
        }
        calculateIndex -= 1
        highlightIndex.append(calculateIndex)
...
//        chatView.register(UINib(nibName: CustomCell.reuseIdentifier, bundle: nil), forCellReuseIdentifier: CustomCell.reuseIdentifier)
        chatView.register(CustomCell.self, forCellReuseIdentifier: CustomCell.reuseIdentifier)
...
//        tableView.deselectRow(at: indexPath, animated: false)
    ...
    func scroll(to row: Int, of table: UITableView) {
        ...
        chatView.scrollToRow(at: IndexPath(row: row, section: 0), at: .middle, animated: true)
        self.view.layoutIfNeeded()
        highlightRow(at: row)
    }
    
    func highlightRow(at row: Int) {
//        let cell = chatView.cellForRow(at: IndexPath(row: row, section: 0))
...
extension ViewController {
    @objc func keyboardWillShow(_ notification: NSNotification) {
        if let keyboardRect = (notification.userInfo?[UIResponder.keyboardFrameEndUserInfoKey] as? NSValue)?.cgRectValue {
            leadingOfTextFieldCS.constant = 16
            keyboardHeight = keyboardRect.height
            self.bottomAlignCollectionViewCS.constant = keyboardHeight - 34
            self.view.layoutIfNeeded()
        }
    }
    ...
    @objc func keyboardWillHide(notification: NSNotification) {
        if let keyboardRect = (notification.userInfo?[UIResponder.keyboardFrameEndUserInfoKey] as? NSValue)?.cgRectValue {
            leadingOfTextFieldCS.constant = 180
            keyboardHeight = keyboardRect.height
